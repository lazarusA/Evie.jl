...
function plotSpectrogram(buf_obs, fs, txt_query, btn_label; colormap = :Bay, marker = :rect)
    ...
    button = Makie.Button(
        fig[1, 1]; label = btn_label, tellheight = false, tellwidth = false,
        halign = 1, valign = 1, buttoncolor = (:grey35, 0.15),
        buttoncolor_hover = :grey65, labelcolor = :silver
    )

    ax_txt_input = Axis(
        fig[1, 1]; width = 250, height = 50,
    ...
    lines!(
        ax, -0.5 .. 0.5, @lift([$buf_obs[n_cut:-1:1]..., $buf_obs[1:n_cut]...] / (2 * length(x)));
    ...
    return display(fig; title = "Evie.jl")
end
